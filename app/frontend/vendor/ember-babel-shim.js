 ...
 *
 * jQuery is loaded as a plain global script in vendor.js. Ensure both `$`
 * and `Ember.$` are assigned before any modules try to call `$.ajax()`.
 */
(function() {
  if (typeof window !== 'undefined' && window.jQuery) {
    // Ensure globals are set for legacy modules that expect `$` and `Ember.$`
    window.$ = window.$ || window.jQuery;
    if (window.Ember && !window.Ember.$) {
